...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Kashly - Professional Wealth Management Platform</title>
    ...
    <body class="antialiased bg-slate-950 text-slate-200 selection:bg-indigo-500/30">
        <div class="min-h-screen hero-gradient flex flex-col">
            <!-- Navigation -->
            <header class="p-6">
                <nav class="max-w-7xl mx-auto flex justify-between items-center glass px-6 py-4 rounded-3xl">
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="text-2xl font-bold tracking-tight text-white">Kashly</span>
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        @if (Route::has('login'))
                            @auth
                                <a href="{{ url('/dashboard') }}" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-indigo-500/20">Dashboard</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-400 hover:text-white transition-colors">Log in</a>
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="px-5 py-2.5 bg-white text-slate-950 hover:bg-slate-200 rounded-xl text-sm font-bold transition-all">Get Started</a>
                                @endif
                            @endauth
                        @endif
                    </div>
            ...
            <main class="flex-1 flex flex-col items-center justify-center px-6 text-center py-20">
                <div class="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-bold uppercase tracking-widest mb-8">
                ...
                <h1 class="text-5xl md:text-7xl font-bold text-white tracking-tight max-w-4xl mb-6">
                    Master Your Wealth, <br/> 
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-emerald-400">Personal & Commercial.</span>
                </h1>
                
                <p class="text-lg md:text-xl text-slate-400 max-w-2xl mb-12 leading-relaxed">
                    Kashly is the first unified financial operating system designed for modern investors. 
                    Manage your personal savings and commercial investment funds in one professional interface.
                </p>

                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    <a href="{{ route('register') }}" class="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold transition-all shadow-2xl shadow-indigo-500/40 text-lg">
                        Start Growing Free
                    </a>
                    <button class="px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white border border-slate-800 rounded-2xl font-bold transition-all text-lg">
                        View Demo
                    </button>
                </div>

                <!-- Floating Elements Preview -->
                <div class="mt-20 w-full max-w-5xl relative">
                    <div class="absolute -top-10 -left-10 w-64 h-64 bg-indigo-600/10 rounded-full blur-3xl"></div>
                    <div class="absolute -bottom-10 -right-10 w-64 h-64 bg-emerald-600/10 rounded-full blur-3xl"></div>
                    
                    <div class="glass p-4 rounded-[2.5rem] shadow-2xl relative z-10 border border-white/5">
                        <img src="https://images.unsplash.com/photo-1611974717483-36009c609930?auto=format&fit=crop&q=80&w=1200" alt="Dashboard Preview" class="rounded-[1.5rem] shadow-lg opacity-80 mix-blend-lighten" />
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent rounded-[2.5rem]"></div>
                    </div>
                </div>
            </main>

            <!-- Footer -->
            <footer class="p-12 border-t border-slate-900 mt-20 text-center">
                <p class="text-slate-500 text-sm">© 2026 Kashly.xyz • Built for the ambitious.</p>
            </footer>
        </div>
    </body>
