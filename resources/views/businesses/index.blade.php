<x-app-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-white tracking-tight">صناديق الاستثمار</h2>
                <p class="text-slate-400 mt-1">إدارة حقائبك الاستثمارية التجارية وشركائك.</p>
            </div>
            <a href="{{ route('businesses.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-semibold transition-all shadow-lg shadow-indigo-500/20 group">
                <svg class="w-5 h-5 ml-2 group-hover:rotate-90 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                إنشاء صندوق جديد
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($businesses as $business)
                <div class="bg-slate-900 p-6 rounded-[2.5rem] border border-white/5 hover:border-indigo-500/30 transition-all group relative overflow-hidden shadow-xl shadow-black/20">
                    ...
                    <div class="flex justify-between items-start mb-8">
                        ...
                        <span class="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-black uppercase tracking-widest">تجاري</span>
                    </div>

                    <h3 class="text-xl font-bold text-white mb-2">{{ $business->name }}</h3>
                    <p class="text-slate-500 text-sm line-clamp-1 mb-8">{{ $business->description ?? 'حقيبة استثمارية' }}</p>

                    <div class="grid grid-cols-2 gap-4 mb-8">
                        <div class="bg-white/5 p-4 rounded-2xl text-center">
                            <p class="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">الشركاء</p>
                            <p class="text-xl font-black text-white">{{ $business->partners_count }}</p>
                        </div>
                        <div class="bg-white/5 p-4 rounded-2xl text-center">
                            <p class="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">الأصول</p>
            ...
            @empty
                <div class="col-span-full py-20 text-center glass rounded-[2rem] border-dashed border-2 border-slate-800">
                    ...
                    <h3 class="text-xl font-bold text-white mb-2">لا توجد صناديق استثمارية</h3>
                    <p class="text-slate-400 mb-8">ابدأ بإنشاء صندوقك الأول لمتابعة الشركاء والأرباح.</p>
