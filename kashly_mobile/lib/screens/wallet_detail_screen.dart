...
class _WalletDetailScreenState extends State<WalletDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final w = data;
    if (w == null) return Scaffold(body: Center(child: Text('فشل تحميل البيانات', style: TextStyle())));

    return Scaffold(
      backgroundColor: Color(0xFFF8FAFC),
      appBar: AppBar(
        title: Text(w['name'], style: TextStyle(fontWeight: FontWeight.w900, color: Colors.indigo.shade900)),
      ...
      child: Column(
        children: [
          Text('الرصيد المتوفر في المحفظة', 
            style: TextStyle(color: Colors.white.withOpacity(0.6), fontWeight: FontWeight.bold, fontSize: 13, letterSpacing: 0.5)),
          SizedBox(height: 12),
          Text('${w?['balance']} ${w?['currency']}', 
            style: TextStyle(color: Colors.white, fontSize: 38, fontWeight: FontWeight.w900, letterSpacing: -1.0)),
          SizedBox(height: 25),
          Row(
  ...
  Widget _buildSmallStat(String label, String val) {
    return Column(
      children: [
        ...
        SizedBox(height: 4),
        Text(val, style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
      ],
    );
  }

  Widget _buildSectionTitle(String title) {
    return Text(title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.blueGrey.shade900));
  }
  ...
  Widget _buildTransactionsList() {
    ...
    if (txs.isEmpty) return Center(child: Padding(
      padding: EdgeInsets.all(30),
      child: Text('لا توجد حركات لهذه المحفظة', style: TextStyle(color: Colors.blueGrey.shade300, fontSize: 14, fontWeight: FontWeight.bold)),
    ));
    return Column(
      children: txs.map((t) {
          ...
          child: Row(
            children: [
              ...
              Expanded(child: Text(t['description'] ?? t['category'] ?? '', 
                style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14, color: Colors.blueGrey.shade800))),
              Text('${isIncome ? '+' : '-'}${t['amount']} ${data?['currency']}', 
                style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: isIncome ? Colors.green.shade600 : Colors.red.shade600)),
            ],
          ),
        );
      }).toList(),
