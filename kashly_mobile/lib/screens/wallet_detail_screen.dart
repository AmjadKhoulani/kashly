...
class _WalletDetailScreenState extends State<WalletDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final w = data;
    if (w == null) return Scaffold(body: Center(child: Text('فشل تحميل البيانات')));

    return Scaffold(
      backgroundColor: Color(0xFFF8FAFC),
      appBar: AppBar(
        title: Text(w['name'], style: TextStyle(fontWeight: FontWeight.w900, color: Colors.lightBlue.shade900)),
  ...
  Widget _buildHeaderCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.lightBlue.shade100.withOpacity(0.5), blurRadius: 30, offset: Offset(0, 15))]
      ),
      child: Column(
        children: [
          Text('الرصيد المتوفر في المحفظة', 
            style: TextStyle(color: Colors.lightBlue.shade400, fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 0.5)),
          SizedBox(height: 15),
          Text('${w?['balance']} ${w?['currency']}', 
            style: TextStyle(color: Colors.lightBlue.shade900, fontSize: 42, fontWeight: FontWeight.w900, letterSpacing: -1.0)),
          SizedBox(height: 25),
          Row(
  ...
  Widget _buildSmallStat(String label, String val) {
    return Column(
      children: [
        Text(label, style: TextStyle(color: Colors.lightBlue.shade400, fontSize: 11, fontWeight: FontWeight.w900)),
        SizedBox(height: 4),
        Text(val, style: TextStyle(color: Colors.lightBlue.shade900, fontSize: 18, fontWeight: FontWeight.w900)),
      ],
    );
  }
      ...
      children: txs.map((t) => Container(
        ...
        child: Row(
          children: [
            ...
            Text('${t['type'] == 'income' ? '+' : '-'}${t['amount']} ${data?['currency']}', 
